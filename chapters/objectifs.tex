...
\section{Objectifs de la thèse}\label{objectifs-de-la-thuxe8se}

L'objectif central de travail a été de développer de nouvelles méthodes
d'apprentissage automatique pour l'analyse de données génomiques, en se
concentrant sur l'intégration de différentes sources de données
biologiques et l'amélioration de l'interprétabilité des modèles.

Partant de l'hypothèse que l'analyse intégrative de données
multi-omiques peut révéler des interactions biologiques complexes, nous
avons formulé les objectifs suivants :

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\{Objectiv\} A main mistake during one's Ph.D.~is to not see the time
passing by. My goal for this Ph.D.~is to be as product-first as I was at
Whitelab. Delivering results quickly \& improving until it is
publishable.
...
RecapPermalink Thus my motto for this Ph.D.~will be:
...
\subsection{Initial Thesis Proposal: Deep learning approaches as
predictors of the cell's regulatory
networks}\label{initial-thesis-proposal-deep-learning-approaches-as-predictors-of-the-cells-regulatory-networks}

\subsubsection{Summary}\label{summary}

Single-cell transcriptomics (scRNA-seq) has revolutionized biology and
medicine by unrevealing the diversity of the cells constituting human
tissues. However, the complex task of inferring gene regulatory networks
has not yet profited from this revolution and tools have not been
predictive enough to reach any usability. Knowing such a network for
each cell type is expected to provide a more comprehensive view of the
cell's states and explain some of the different cellular phenotypes and
responses of the cells.

The aim of this Ph.D.~project is to develop novel approaches using deep
...
This PhD project will impact both computational fields and biomedical
fields, by developing rigorous methods that can reliably maximize the
information extracted from complex multimodal datasets. In particular,
applications of the tool will contribute to personalized medicine.

\subsubsection{Background and
objectives}\label{background-and-objectives}
...
\subsubsection{Detailed description of the
project}\label{detailed-description-of-the-project}
...
\paragraph{WP1: Review of current tools and creation of a set of
...
\paragraph{WP2: GNN model/layers to better predict TF-gene
...
\paragraph{WP3: Collaboration to test the model's prediction on novel
...
\subsection{Objectif 1: assessment and review of existing
...
\subsection{Objectif 2: novel model and
...
\subsection{Objectif 3: novel architecture and approaches for training
...
\subsection{Objectif 4: General improvements demonstrating novel
