...
Recently, using specific neural network architectures with great scaling properties and training on a large swath of the internet, we built what has been termed Large Language Models (\gls{LLM}s). \gls{LLM}s have become ubiquitous nowadays, and researchers have been trying to extend  them to novel data modalities in what has been termed \textbf{world models}\cite{haWorldModels2018}. These models, trained on physics simulations, videos or other modalities, aim to predict the next state of complex physical phenomena, such as what happens when someone throws a ball or the physics of water flowing down a river.

World models have already shown promise in powering, for example, humanoid robots. In a different domain, Google DeepMind recently released a model for predicting the weather better and faster than the best "physics-based" models\cite{lamLearningSkillfulMediumrange2023}. But why and how are these models so powerful? While this remains an active area of research, essential theories have been presented. 
...
Another enabler to achieve such large models was the advent of Graphical Processing Units (\gls{GPU})s, while allowed large and very efficient parallel matrix operations. Thanks to  interconnects, hundreds of GPUs could be linked to form nodes and pods of compute clusters which allowed training distributed manner, such models could be scaled to orders of magnitude more parameters, thereby demonstrating the power of weak learning methods\cite{lecunDeepLearning2015}.
