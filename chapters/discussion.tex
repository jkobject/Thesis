...
The main challenge that remains to be solved for better single-cell foundation models is data. Indeed, while we have amassed hundreds of millions of cells from public datasets during this thesis, we have noticed many limitations in three important axes: diversity, and quality.
...
\begin{figure}[ht]
  ...
  \caption{The number of genes detected per reads for different single-cell technologies. Adopted from \citet{simoneComparativeAnalysisCommercial2024}.}
  \label{fig:sequal}
\end{figure}
...
Indeed, these two modalities, spatial and dissociated single cell, exist within a breadth of other single cell modalities. Each modality often uncovers a different aspect of cellular biology, and integrating these diverse data types is crucial to make sense of RNA biology itself \cite{samaranScConfluenceSinglecellDiagonal2024}. One limitation of this thesis was to focus only on \gls{scRNA-seq} data. Common other modalities include single-cell ATAC-seq, and surface protein sequencing.

Moreover, another axis of information relate to the temporal evolution of cells with or without interventions such as perturbations. Again, mostly due to a lack of diverse and good quality data, at foundation model-scale, we did not focus much on this axis either. But such data will be required for the model to learn causality, especially when assessed at multiple fine-grained timescales and in richer cellular models such as organoids. Currently, only imaging-based methods can achieve the high throughput, low-cost and multi-time point at single cell level for getting information at scale \cite{fayRxRx3PhenomicsMap2023}. However many challenges remain, mostly in term of indexing of cells across time, perturbations, and modalities.
...
Many challenges remain in bridging fields such as data engineering, machine learning, material engineering, microelectronics, molecular biology, and cell biology, with better interpretability of the models \cite{pedrocchiSparseAutoencodersReveal2025} but the rewards are tremendous, as many diseases won't be solved by brute force approach or by targeting one specific gene, whether cancer or other complex multi-cellular aging diseases.
