...
\caption[List of novelties in scPRINT and comparison to scGPT and scFoundation]{Comparison of the features and novelties from scPRINT compared to 2 similar published state-of-the-art methods: scGPT and scFoundation.}\label{tab:s1-novelties}\\
\toprule\noalign{}
...
\caption[Model comparison]{Comparing different model sizes and architectures. Comparing scPRINT to other state-of-the-art methods, as well as GPT2-small and GPT3-large models.}
\label{table-s2-model-comparison}
\end{table}
...
\caption[Ablation study and impact on performance across tasks]{The table shows the results of the ablation study on denoising, embedding with batch correction, and cell-type classification tasks. Results are displayed for the medium-size scPRINT model. Top to bottom: \emph{small, medium, large:} regular models of various sizes. \emph{medium-nofreeze:} a model trained without freezing gene embedding during pre-training. \emph{medium-2-heads:} a model trained with only two heads per layer instead of 4. \emph{medium-MSE}: a model with Mean Squared Error instead of the ZINB loss. \emph{medium-MVC:} a model trained with scGPT's MVC methodology for the creation of the cell embedding. medium-noPE: a model trained without positional encoding for the gene's location. medium-no-random-weighted: a model trained without weighted random sampling. \emph{medium-MLM}: a model trained with masked language modeling instead of denoising.}
\label{table-s3-ablation-study-and-impact-on-performance-across-tasks}
\end{table}
...
\caption[Computational speed of various GN inference methods]{The computational speed of running various gene network inference methods on a set of 4000 genes and 1000 cells. It is showing that transformer-based models are far faster than previous methods, owing to their clever use of the GPU and pre-training.}\label{tab:s4-computational-speed}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
\textbf{model}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\begin{longtable}[H]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1867}}@{}}
\caption[Performance of GN inference methods on the Sergio simulated scRNAseq dataset]{We generate a Sergio simulated scRNAseq dataset of 1000 cells for 800 genes from the RegNetwork ground truth network. We here showcase the ability of each model to recover the RegNetwork ground truth from this dataset. It shows how only scPRINT can recover some of RegNetwork's connections.}\label{tab:s5-sergio}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
\textbf{model}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{./supplementaries/scprint_media/table6-1.png}
\caption[Comparison scPRINT model size on performance across tasks and GN inference abilities]{Comparison of scPRINT model sizes on performance across tasks and GN inference abilities.}
\label{fig:s6-model-size}
\end{figure}

\includegraphics[width=\linewidth]{./supplementaries/scprint_media/table6-2.png}
\caption[Comparison scPRINT model size on performance across tasks and GN inference abilities. (part2)]{Comparison of scPRINT model sizes on performance across tasks and GN inference abilities.}
\label{fig:s6-model-size-2}
\end{figure}
...
\caption[Overlap of different GN ground truths]{Comparison of the overlap, expressed as precision and recall, of the three different ground truth networks used: MCalla, Omnipath, and gwps.}\label{tab:s7-overlap}\\
\toprule\noalign{}
...
\caption[Omnipath benchmark results on the genome-wide perturb-seq dataset]{Omnipath network overlap (EPR, AUPRC), as well as transcription factor enrichment, TF target enrichment, and cell type marker enrichment for gene networks generated by the different tools on the genome-wide perturb seq K562 cells at steady state (no perturbations).}\label{tab:s8-omnipath-gwps}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
\textbf{tool}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\caption[Number of labels predicted by the model for each class]{Number of labels predicted by the model for each class. We use hierarchical classification for cell type, disease, assay, and ethnicity.}\label{tab:s13-predictions}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
ethnicity
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\caption[Visualization of human gene embedding from ESM2]{Umap of the ESM2 protein embeddings for the most common protein of all protein-coding genes in Ensembl. The PCA variance ratio is 0.856 for the top 50 principal components. We color it using the Louvain clustering of the embedding.}
\label{fig:s1-esm2-embedding}
\end{figure}
...
\caption[Gene network inference comparison with Omnipath per datasets]{The same plots as in Figures 2B, C, and D, showing the Omnipath and enrichment results per dataset for each of the 3 datasets used. Source data are provided as a Source Data file.}
\label{fig:s2-omnipath-per-dataset}
\end{figure}
...
\caption[Performance of each GN inference method on predicting the TF-gene only subset of the GWPS ground truth network]{Performances of each model's networks on its overlap with the TF-gene-only subset of the genome-wide perturb seq ground truth. It shows that on this task, most foundation models do not perform well. This could be due to the way their attention matrix is normalized. Source data are provided as a Source Data file.}
\label{fig:s4-tf-gene-performance}
\end{figure}
...
\caption[In-depth view of the BPH dataset and its scPRINT-predicted annotations]{Detailed view of the assay, development stage, scPRINT-predicted diseases, and scPRINT-predicted cell types. Predicted diseases and cell types have been ``cleaned'' following the strategy presented in Figure 5. We also add pie charts of the relative abundance of each predicted label.}
\label{fig:s9-bph-annotations}
\end{figure}
...
\caption[Differential expression analysis of the B-cell cluster vs the rest of the cells in the BPH dataset]{Top genes of the differential expression analysis of the scPRINT inferred B-cell cluster in vs the rest of the cells in the BPH dataset.}
\label{fig:s10-bcell-diff-expr}
\end{figure}
...
\caption[Gene enrichment comparison in the PAGE4 GN]{Comparison of the top 20 most enriched terms in Wikipathways, GO molecular function, and Reactome for the 40 most connected genes to PAGE4 in both BPH-associated and normal fibroblast GNs inferred by scPRINT.}
\label{fig:s11-page4-enrichment}
\end{figure}
...
\caption[Gene Network enrichment comparison between the BPH and normal fibroblast on their Louvain communities]{Dotplot of the top 20 GO Molecular function gene sets enriched in the Louvain communities of the BPH and normal fibroblast's Gene Networks.}
\label{fig:s12-gn-enrichment}
\end{figure}
...
\caption[Graphical Model]{Schematic representation of the zero-inflated negative binomial graphical model of the expression decoder. We generate three values $\mu,\ \theta,\ \pi$ which are used to model a distribution. We also multiply the $\mu$ with the depth (or total count) over the cell.}
\label{fig:s13-graphical-model}
\end{figure}
...
\caption[Hierarchical classifier]{Schematic representation of the hierarchical classifier and its behavior during training. We can train on labels not predicted by the classifier as long as they are parent to one of the predicted labels in the ontological tree.}
\label{fig:s14-hierarchical-classifier}
\end{figure}
...
\caption[Detailed representation of the bottleneck learning procedure]{Schematic representation of the bottleneck learning procedure where scPRINT's Bidirectional Transformer Encoder is used both as the ``Encoder'' and ``Decoder'' of an auto-encoding (AE) bottleneck learning scheme.}
\label{fig:s15-bottleneck}
\end{figure}
