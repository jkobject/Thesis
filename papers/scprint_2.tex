...
\chapter[scPRINT-2: Towards the next-generation of cell foundation models and benchmarks]{scPRINT-2: Towards the next-generation of cell foundation models and benchmarks}
\label{article3}
...
present \gls{scPRINT}-2, a single-cell Foundation Model pretrained across 350
million cells and 16 organisms. Our contributions in pretraining tasks,
...
results. Finally, thanks to our pretraining database, we uncover
...
Open-sourcing not just model weights but their pretraining tasks and
...
also showcasing pretraining strategies, data encoding, and
decoding\citep{scprint}. scPRINT was trained on 50 million cells
using a multitask pretraining strategy that included expression
...
architecture and pretraining tasks across 42 different scenarios. In
...
pretraining datasets, compiling a 350-million-cell database---the
...
data augmentation approaches. It uses a set of updated pretraining
...
\gls{scPRINT}-2, its dataloader, pretraining datasets, preprocessing, task
functions, pretrained weights, as well as the additive benchmark
...
of \gls{scFM}s, including pretraining databases, architectures, and training
...
\begin{figure}[H]
    ...
    \caption[Presentation of the \gls{scPRINT}-2 model, pretraining dataset, and additive benchmark]{Presentation of the \gls{scPRINT}-2 model, pretraining dataset, and additive benchmark. (a) The additive benchmark example table with its gymnasium scores across the \gls{scFM}'s features. (b) Our \gls{scPRINT}-2 corpus pretraining dataset, with 16 organisms across 300+ tissues. \gls{UMAP} of 15 million cells from the corpus integrated using \gls{scPRINT}-2. Colors represent species. (c) The \gls{scPRINT}-2 model, its input data, and its different outputs. Source data are provided as a Source Data file.}
...
method. The base model's pretraining task uses a 30\% gene expression
mask. We pretrain the models 6 times across multiple seeds to generate
...
pretraining task for single-cell data in classification and embedding
...
good supplement to the pretraining task, as without it, we observe a
...
selection in pretraining \gls{scFM}s. Indeed, when using the Tahoe-100M
database solely for pretraining, the model's overall performance
...
pretraining\citep{comprehensiveSurveyDataAugmentation2024}.
...
\subsubsection{scPRINT-2 pretraining
corpus}\label{scprint-2-pretraining-corpus}
...
model did not sample as many cells as actually exist in the pretraining
...
distinct input cell profiles during pretraining, corresponding to 2000
...
datasets more often during pretraining, since it is trained only on
...
these organisms in the pretraining dataset (see Figure \ref{fig:scprint2_overview}). For a horse
...
pretrained on only 3200 genes, demonstrating generalization to larger
...
pretraining. This can be done, for example, by creating
...
Figure~\ref{fig-s14-spatial-plot-of-the-xenium-melanoma-dataset-with-scprint-2-predicted-cell-labels}). It likely reflects the biases of the pretraining dataset,
...
pretraining while maintaining the ability to work with unseen
...
We have also presented the largest pretraining database to date,
...
inference, and working with organisms unseen during pretraining. We
...
principal pretraining bottlenecks, and efforts will be needed to
