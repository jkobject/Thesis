...
\(\mathbf{e}_{\mathbf{i,j}}\  = \ MLP(log_{2}(x_{i,j} + 1)),\ x_{i,j}\mathfrak{\in \ R\ ,\ }\mathbf{e}_{\mathbf{i,j}}\mathfrak{\  \in \ R}\ ^{d}\),
(1)
...
\(Dropout(ReLU(LayerNorm(Linear(\mathbf{e}_{\mathbf{i,j}}\ ))))\) , (2)
...
\(X_{i}\  = \lbrack\mathbf{g}_{\mathbf{0}} + \mathbf{e}_{\mathbf{i,0}} + \mathbf{l}_{\mathbf{0}}\ ,\ \mathbf{g}_{\mathbf{1}} + \mathbf{e}_{\mathbf{i,1}} + \mathbf{l}_{\mathbf{1}}\ ,\ ...\ ,\mathbf{e}_{\mathbf{i,t}}\ ,\mathbf{p}_{\mathbf{default}},\ \mathbf{p}_{\mathbf{celltype}}\ ,\mathbf{p}_{\mathbf{disease}}\ ,\ ...\rbrack\),
(3)
...
\(\mathbf{e}_{\mathbf{t,i}}\  = \ MLP(log_{2}(1 + t_{i}))\), where
\(t_{i} = \ \sum_{j}^{}x_{i,j}\), (4)
...
\(NB(x\ |\ \mu,\ \theta) = \ \frac{\Gamma(x + \theta)}{x!\Gamma(\theta)}{(\frac{\mu}{\mu + \theta})}^{x}{(\frac{\theta}{\mu + \theta})}^{\theta}\)
, (8)
...
\(\mu,\ \pi\) (see ~\ref{sec:suppscprint}. Supplementary Figure S13)
...
this represents around 400 different labels (see ~\ref{sec:suppscprint}. Supplementary Table S13).
...
\widehat{\mathbf{c}}\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ if\ \left\{ i\ |\ c_{i} = 1 \right\}\  \subseteq T\  \\
LSE\left( {\widehat{\mathbf{c}}}_{d} \right)||{\widehat{\mathbf{c}}}_{\sim d}\ \ \ \ else\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 
\end{array} \right.\ \) (12)
...
labels (see ~\ref{sec:suppscprint}. Supplementary Figure S14)
...
MONDO, EFO, HANCESTRO-126}, respectively.
...
\(L\left( x \middle| \mu,\theta,\pi \right) = \ \left\{ \begin{array}{r}
\frac{\pi}{\pi - \ \theta \bullet (\log(\theta) - log(\theta + \mu))}\ \ \ \ \ \ \ \ \ if\ x = 0 \\
\frac{{(\frac{\mu}{\theta + \mu})}^{x} \cdot \Gamma(x + \theta) \bullet \sigma( - \pi)}{exp(\pi) \bullet \left( \frac{\mu}{\theta + \mu} \right)^{\theta} \bullet \Gamma(\theta) \bullet \Gamma(x + 1)}\ \ if\ x\  > 0
\end{array} \right.\ \) (17)

with \(\sigma\) the sigmoid function.

We show that models trained with such a framework perform better than
regular MSE-trained models (see ~\ref{sec:suppscprint}. Supplementary Table S3), for which one only outputs
...
\(x_{i,j} = \ \mathbf{e}_{\mathbf{i}} \odot \mathbf{g}_{\mathbf{j}}\mathbf{\ }\ \),
(18)
...
this methodology than ours (see ~\ref{sec:suppscprint}. Supplementary Table S3).
...
(see ~\ref{sec:suppscprint}. SupplementaryFigure S15).

This means the encoder is considered as

\(\mathbf{e}_{\mathbf{A,i}} = scPRINT(\lbrack\mathbf{g}_{\mathbf{O}} + \mathbf{e}_{\mathbf{0,i}} + \mathbf{l}_{\mathbf{0}},{\ \mathbf{g}}_{\mathbf{1}} + \mathbf{e}_{\mathbf{1,i}} + \mathbf{l}_{\mathbf{1}},\ ...,\ \mathbf{p}_{\mathbf{A}}\rbrack)\),
(19)
...
\(\mathbf{\mu}_{\mathbf{i}},\mathbf{\theta}_{\mathbf{i}},\mathbf{\pi}_{\mathbf{i}} = scPRINT(\lbrack\mathbf{g}_{\mathbf{O}} + \mathbf{l}_{\mathbf{0}},\mathbf{\ g}_{\mathbf{1}} + \mathbf{l}_{\mathbf{1}},\ ...\rbrack,\ \mathbf{e}_{\mathbf{0,i}},\ \mathbf{e}_{\mathbf{1,i}},\ ...,\ \mathbf{e}_{\mathbf{t,i}})\),
(20)
...
\(Loss_{bottleneck}\  = \sum_{i = 0}^{m}{Loss_{ZINB}(\mathbf{x}_{\mathbf{i}}\mathbf{|}\ \mathbf{\mu}_{\mathbf{i}}\mathbf{\ },\ \mathbf{\theta}_{\mathbf{i}}\mathbf{\ },\ \mathbf{\pi}_{\mathbf{i}})}\ \),
(21)
...
\(Loss_{constrastive} = \frac{1}{m^{2}}\sum_{i = 1}^{m}{\sum_{i'}^{m}{1 - cos(\mathbf{e}_{\mathbf{i}},\mathbf{e}_{\mathbf{i'}})}}\),
(22)
...
removal of the batch effects (see ~\ref{sec:suppscprint}. SupplementaryFigure S7).
...
(see ~\ref{sec:suppscprint}. SupplementaryFigure S16).
...
\(softmax(\frac{\mathbf{QK}^{T}}{\sqrt{d_{k}}})\). (24)
...
\(softmax(\frac{{mean_{U}(\mathbf{Q})_{}mean_{U}(\mathbf{K})}^{T}}{\sqrt{d_{k}}})\).
(25)
...
elements connected to a sparse set of other elements (see ~\ref{sec:suppscprint}. SupplementaryFigure
S3). Due to the sparsity of connections for regulators (i.e., sources)
...
\(P(X = x) \approx \frac{\Gamma(x + \theta)}{x!\Gamma(\theta)}1^{\theta}{(\frac{\mu}{\theta + \mu})}^{x}\)
(26)
...
\(\Gamma(x + \theta) \approx \sqrt{2\pi(x + \theta)}{(\frac{x + \theta}{e})}^{x + \theta}\)
(27)

\(\Gamma(\theta) \approx \sqrt{2\pi\theta}{(\frac{\theta}{e})}^{\theta}\)
(28)

Simplifying the ratio of the Gamma functions:

\(\frac{\sqrt{2\pi(x + \theta)}{(\frac{x\  + \ \theta}{e})}^{x\  + \ \theta}}{\sqrt{2\pi\theta}{(\frac{\theta}{e})}^{\theta}}\  = \sqrt{\frac{x\  + \ \theta}{\theta}}{(\frac{x\  + \ \theta}{\theta})}^{\theta}{(\frac{x\  + \ \theta}{e})}^{x}\ \).
(29)
...
\({P(X = x) \approx \frac{1}{x!}(\frac{\mu}{\theta + \mu})}^{x}{(\frac{\theta\  + \ x}{\theta})}^{x}\)
(30)
