...
\gls{scPRINT} \citep{scprint} is a foundation model trained on more than 50 million unique single-cell \gls{RNA-seq} profiles, representing around 100B tokens. It learns with a multi-task pretraining loss, allowing \gls{SOTA} zero-shot abilities in denoising and label prediction. \gls{scPRINT} builds on previous foundation models, like \gls{scGPT} \citep{cuiScGPTBuildingFoundation2024} and scFoundation \citep{haoLargescaleFoundationModel2024}. It improves upon them on multiple benchmarks and is also easier to use and faster to train than many other similar models. Additionally, it comes with a gymnasium of benchmarks presented in \citet{scprint}. For these reasons, we chose to use it as our \gls{cFM} and the starting point for our work.
...
We propose going beyond simply reusing lower-scale models' representations and fine-tuning them during the pretraining of the upper-scale model using an adapter layer (see Figure~\ref{fig:first}B). With such layer, each output embedding $\ve$ is transformed with a differentiable function $f$ (here, an \gls{MLP}):
